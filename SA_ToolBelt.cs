    ...
    public partial class SAToolBelt : Form
    {
        ...
        private void DockConsole()
        {
            try
            {
                ...
                {
                    // Get the RichTextBox from the floating ConsoleForm
                    var consoleRichTextBox = _consoleForm.GetConsoleRichTextBox();

                    if (consoleRichTextBox != null)
                    {
                        // Remove from ConsoleForm
                        _consoleForm.RemoveConsoleControl();

                        // Add to the tab
                        tabConsole.Controls.Clear();
                        tabConsole.Controls.Add(consoleRichTextBox);

                        // Set docking and context menu
                        ...
                        if (!tabControlMain.TabPages.Contains(tabConsole))
                        {
        ...
        private void UndockConsole()
        {
            try
            {
                if (_consoleIsDocked && _consoleForm != null)
                {
                    // Get the RichTextBox from the tab
                    var consoleRichTextBox = tabConsole.Controls.OfType<RichTextBox>().FirstOrDefault();

                    if (consoleRichTextBox != null)
                    {
                        // Remove from tab
                        tabConsole.Controls.Remove(consoleRichTextBox);

                        // Add back to ConsoleForm
                        _consoleForm.AddConsoleControl(consoleRichTextBox);
        ...
        */
        ...
        private void btnLdapGenerate_Click(object sender, EventArgs e)
        {
            ...
            {
                _consoleForm.WriteError($"Error in Generate button click: {ex.Message}");
        ...
        /// </summary>
        private void btnLdapGetUid_Click(object sender, EventArgs e)
        {
            try
            {
            ...
            {
                _consoleForm.WriteError($"Error in Get UID button click: {ex.Message}");
                // Show the button again if there was an error
                btnLdapGetUid.Visible = true;
            }
        }
        ...
        /// </summary>
        private void btnLdapClearForm_Click(object sender, EventArgs e)
        {
            try
            {
                ClearLdapForm();
        ...
        private async void btnLdapCreateAccount_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                // Validate all required fields (uses existing ValidateLdapFormFields method)
                if (!ValidateLdapFormFields())
                {
                    return; // Validation method shows its own error messages
                }
                ...
                _consoleForm.WriteInfo($"Starting LDAP account creation for user: {ntUserId}");

                // Step 1: Create LDAP user account
                await Task.Run(() =>
                {
                ...
                _consoleForm.WriteSuccess("LDAP user account created successfully.");

                // Step 2: Add to DEFAULT security group in Directory Services (RHDS)
                string selectedGroup = cbxDefaultSecurityGroups.SelectedItem?.ToString();
                bool shouldAddToGroup = false;

                if (!string.IsNullOrEmpty(selectedGroup) &&
                    !selectedGroup.Contains("(None") &&
                    !selectedGroup.Contains("(No Security") &&
                    !selectedGroup.Contains("(Error"))
                {
                    shouldAddToGroup = true;

                    try
                    {
                        _consoleForm.WriteInfo($"Adding user to Directory Services group: {selectedGroup}");

                        string securityGroupsOU = GetSecurityGroupsOU();

                        await Task.Run(() =>
                        {
                            bool success = _rhdsService.AddUserToDSGroup(ntUserId, selectedGroup, securityGroupsOU);

                            if (success)
                            {
                                _consoleForm?.WriteSuccess($"Added {ntUserId} to Directory Services group: {selectedGroup}");
                            }
                            else
                            {
                                _consoleForm?.WriteWarning($"Failed to add {ntUserId} to Directory Services group: {selectedGroup}");
                            }
                        });
                    }
                    catch (Exception groupEx)
                    {
                        _consoleForm.WriteWarning($"User created but failed to add to Directory Services group: {groupEx.Message}");
                        MessageBox.Show(
                            $"User account created successfully, but failed to add to Directory Services group '{selectedGroup}'.\n\n{groupEx.Message}\n\nYou may need to add the user to the group manually.",
                            "Partial Success",
                            MessageBoxButtons.OK,
                            MessageBoxIcon.Warning);
                    }
                ...
                {
                    _consoleForm.WriteInfo("No security group selected - skipping group assignment.");
                }

                // Step 3: Wait for AD replication and add to AD group
                _consoleForm.WriteInfo("Waiting for Active Directory replication...");
                _consoleForm.WriteInfo("Security groups do NOT replicate automatically - will add to AD group manually once user is found.");

                bool userFoundInWindows = false;

                for (int attempt = 1; attempt <= 3; attempt++)
                {
                    _consoleForm.WriteInfo($"Checking for user in Windows AD - Attempt {attempt} of 3...");

                    await Task.Delay(3000); // Wait 3 seconds between attempts

                    if (_adService.UserExists(ntUserId))
                    {
                        userFoundInWindows = true;
                        _consoleForm.WriteSuccess($"User {ntUserId} found in Windows Active Directory!");
                        break;
                    }

                    if (attempt < 3)
                    {
                        _consoleForm.WriteInfo("User not found yet, waiting 3 seconds before retry...");
                    }
                }

                // Step 4: Add user to the SAME group in AD (if user was found and a group was selected)
                if (userFoundInWindows && shouldAddToGroup)
                {
                    try
                    {
                        _consoleForm.WriteInfo($"Adding user to Active Directory group: {selectedGroup}");

                        // Use AD to add user to group
                        using (var context = new PrincipalContext(ContextType.Domain))
                        {
                            using (var user = UserPrincipal.FindByIdentity(context, ntUserId))
                            {
                                if (user != null)
                                {
                                    using (var group = GroupPrincipal.FindByIdentity(context, selectedGroup))
                                    {
                                        if (group != null)
                                        {
                                            group.Members.Add(user);
                                            group.Save();
                                            _consoleForm.WriteSuccess($"Added {ntUserId} to Active Directory group: {selectedGroup}");
                                        }
                                        else
                                        {
                                            _consoleForm.WriteWarning($"Active Directory group not found: {selectedGroup}");
                                            _consoleForm.WriteWarning($"User will need to be manually added to {selectedGroup} in Active Directory.");
                                        }
                                    }
                                }
                                else
                                {
                                    _consoleForm.WriteError($"User {ntUserId} found but could not be loaded from AD.");
                                }
                            }
                        }
                    }
                    catch (Exception adGroupEx)
                    {
                        _consoleForm.WriteWarning($"Failed to add user to Active Directory group: {adGroupEx.Message}");
                        _consoleForm.WriteWarning($"User will need to be manually added to {selectedGroup} in Active Directory.");
                    }
                }
                else if (!userFoundInWindows && shouldAddToGroup)
                {
                    _consoleForm.WriteWarning($"User {ntUserId} not found in Windows AD after 3 attempts.");
                    _consoleForm.WriteWarning($"Once user appears in AD, manually add them to the '{selectedGroup}' group in Active Directory.");

                    MessageBox.Show(
                        $"User account '{ntUserId}' created successfully in LDAP and added to Directory Services group '{selectedGroup}'.\n\n" +
                        $"However, the user was not found in Windows AD after 3 attempts.\n\n" +
                        $"Once the user appears in Active Directory, you will need to MANUALLY add them to the '{selectedGroup}' group in AD.",
                        "Manual AD Group Assignment Required",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Warning);
                }

                // Success message
                _consoleForm.WriteSuccess($"Account creation process completed for user: {ntUserId}");

                if (userFoundInWindows && shouldAddToGroup)
                {
                    MessageBox.Show(
                        $"User account '{ntUserId}' has been created successfully!\n\n" +
                        $"✓ LDAP account created\n" +
                        $"✓ Added to Directory Services group: {selectedGroup}\n" +
                        $"✓ Replicated to Windows AD\n" +
                        $"✓ Added to Active Directory group: {selectedGroup}\n\n" +
            ...
            {
                _consoleForm.WriteError($"Error creating LDAP account: {ex.Message}");
        ...
        private void CreateOUConfigurationCSV()
        {
            try
            {
                ...
                // Create CSV with headers
                string csvContent = "ou,middleName\n";
        ...
        private void AddOUToCheckedListBox(string ou, string middleName)
        {
            switch (middleName.ToLower())
            {
                ...
                case "patriotpark":
                    ...
                    break;
                case "windows":
                    if (!cbxListWindowsOu.Items.Contains(ou))
                        cbxListWindowsOu.Items.Add(ou, false);
                    break;
                case "gangs":
        ...
        private void SaveOUConfigurationToCSV()
        {
            try
            {
                ...
                var csvContent = new StringBuilder();
                csvContent.AppendLine("ou,middleName");
                ...
                AddOUsFromCheckedListBox(csvContent, cbxListWindowsOu);
        ...
        private void AddOUsFromCheckedListBox(StringBuilder csvContent, CheckedListBox checkedListBox)
        {
            ...
            foreach (string ou in checkedListBox.Items)
            {
                ...
                csvContent.AppendLine($"{escapedOU},{middleName}");
            }
        }
        ...
        private void btnAddWindowsOu_Click(object sender, EventArgs e)
        {
            AddOUToConfiguration(cbxListWindowsOu);
        }
        ...
        {
            AddOUToConfiguration(cbxListSecurityGroupsOu);
        }
        ...
        private void btnRemoveSelectedOus_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                totalRemoved += RemoveSelectedOUsFromCheckedListBox(cbxListWindowsOu);
        ...
        private async Task LoadOnlineOfflineTabAsync()
        {
            try
            {
                ...
                await LoadComputersToListBoxAsync(cbxListWorkStationOu, lbxWindows, "Workstations");
                ...
                await LoadComputersToListBoxAsync(cbxListWindowsOu, lbxWindows, "Windows");
        ...
        private async Task CheckAllOnlineOfflineStatusAsync()
        {
            try
            {
                ...
                allComputers.AddRange(lbxWindows.Items.Cast<string>());
        ...
        private void UpdateOnlineOfflineStatus(Dictionary<string, bool> onlineResults)
        {
            ...
            UpdateListBoxStatus(lbxWindows, onlineResults);
